...
parser.add_argument(
    "--batchsize", type=int, default=256, help="batch size used for pretraining"
)
# mnist=50000, ytf=6700, coil100=5000, reuters10k=50000, yale=10000, rcv1=6100. This amounts to ~200 epochs.
parser.add_argument(
    "--niter", type=int, default=50000, help="number of iterations used for pretraining"
)
...
parser.add_argument(
    "--lr", default=10, type=float, help="initial learning rate for pretraining"
)
...
parser.add_argument(
    "--level", default=0, type=int, help="index of the module to resume from"
)
parser.add_argument(
    "--data", dest="db", type=str, default="mnist", help="name of the dataset"
)
...
parser.add_argument(
    "--h5", dest="h5", help="to store as h5py file", default=False, type=bool
)
parser.add_argument(
    "--tensorboard", help="Log progress to TensorBoard", action="store_true"
)
parser.add_argument(
    "--id", type=int, help="identifying number for storing tensorboard logs"
)
...
def main(args):
    ...
    nepoch = int(
        np.ceil(np.array(args.niter * args.batchsize, dtype=float) / len(trainset))
    )
    step = int(
        np.ceil(np.array(args.step * args.batchsize, dtype=float) / len(trainset))
    )

    trainloader = torch.utils.data.DataLoader(
        trainset, batch_size=args.batchsize, shuffle=True, **kwargs
    )
...
def pretrain(args, outputdir, params, use_cuda, trainloader, testloader, logger):
    ...
    for index in range(startlayer, numlayers + 1):
        ...
        bias_params = filter(
            lambda x: ("bias" in x[0]) and (x[1].requires_grad), net.named_parameters()
        )
        bias_params = list(map(lambda x: x[1], bias_params))
        nonbias_params = filter(
            lambda x: ("bias" not in x[0]) and (x[1].requires_grad),
            net.named_parameters(),
        )
        nonbias_params = list(map(lambda x: x[1], nonbias_params))

        optimizer = optim.SGD(
            [{"params": bias_params, "lr": 2 * lr}, {"params": nonbias_params}],
            lr=lr,
            momentum=0.9,
            weight_decay=params["wdecay"][0],
            nesterov=True,
        )
